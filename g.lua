...
do -- Example UI
	local Window = library:AddWindow("Kakah menu legit V2", {
		main_color = Color3.fromRGB(41, 74, 122),
		min_size = Vector2.new(500, 430),
		toggle_key = Enum.KeyCode.RightShift,
		can_resize = true,
	})
	local Tab = Window:AddTab("Main")
	local Tab2 = Window:AddTab("Visuals")
	local Tab3 = Window:AddTab("Experimental")
	...
	do -- Elements
		...
		getgenv().WallHack.Crosshair.Transparency = 0
		getgenv().WallHack.Crosshair.Enabled = false
		getgenv().Aimbot.Settings.SendNotifications = false
		getgenv().Aimbot.Settings.SaveSettings = false
		getgenv().WallHack.Settings.SendNotifications = false
		getgenv().WallHack.Visuals.HeadDotSettings.Enabled = false
		getgenv().WallHack.Visuals.ESPSettings.TextColor = "255, 255, 255"
		...
		getgenv().WallHack.Visuals.HeadDotSettings.Color = "255, 255, 255"
		getgenv().WallHack.Visuals.BoxSettings.Color = "255, 255, 255"
		getgenv().WallHack.Visuals.TracersSettings.Color = "255, 255, 255"

		getgenv().Aimbot.Settings.TriggerKey = "MouseButton2"

		local Switch = Tab:AddSwitch("Aimbot", function(bool)
			getgenv().Aimbot.Settings.Enabled = bool
		end)
		Switch:Set(false) 

		local Switch = Tab:AddSwitch("Wallcheck (might be laggy)", function(bool)
			getgenv().Aimbot.Settings.WallCheck = bool
		end)	
		Switch:Set(false)

		local Switch = Tab:AddSwitch("Alivecheck", function(bool)
			getgenv().Aimbot.Settings.AliveCheck = bool
		end)	
		Switch:Set(false)

		local Switch = Tab:AddSwitch("Teamcheck", function(bool)
			getgenv().Aimbot.Settings.TeamCheck = bool
		end)	
		Switch:Set(false)

		local Slider = Tab:AddSlider("Smoothness", function(x)
			getgenv().Aimbot.Settings.Sensitivity = x
		...
		local Dropdown = Tab:AddDropdown("Aimpart", function(object)
			getgenv().Aimbot.Settings.LockPart = object
		end)
		local obj = Dropdown:Add("Head")
		local obj = Dropdown:Add("Torso")
		local obj = Dropdown:Add("HumanoidRootPart")


		
		local Switch = Tab2:AddSwitch("Tracers", function(bool)
			Environment.Visuals.TracersSettings.Enabled = bool
		end)
		Switch:Set(false)

		local Switch = Tab2:AddSwitch("Name esp", function(bool)
			Environment.Visuals.ESPSettings.Enabled = bool
		end)
		Switch:Set(false)
	

		local Switch = Tab2:AddSwitch("Boxes esp", function(bool)
			Environment.Visuals.BoxSettings.Enabled = bool
		end)
		Switch:Set(false)

		local Switch = Tab2:AddSwitch("Alivecheck", function(bool)
			Environment.Settings.AliveCheck = bool
		end)
		Switch:Set(false)

		local Switch = Tab2:AddSwitch("Teamcheck", function(bool)
			Environment.Settings.TeamCheck = bool
		end)
		Switch:Set(false)

		local Switch = Tab2:AddSwitch("Circle filled", function(bool)
			getgenv().Aimbot.FOVSettings.Filled = bool
		end)
		Switch:Set(false)

		local Slider = Tab2:AddSlider("Fov radius", function(x)
			getgenv().Aimbot.FOVSettings.Amount = x
			u.Radius = x;
		end, { -- (options are optional)
			["min"] = 10, -- Default: 0
			["max"] = 800, -- Default: 100
		...
		local Slider = Tab2:AddSlider("Transparency", function(x)
			getgenv().Aimbot.FOVSettings.Transparency = x
		end, { -- (options are optional)
			["min"] = 0, -- Default: 0
			["max"] = 1, -- Default: 100
			["readonly"] = false, -- Default: false
		...
		local Slider = Tab2:AddSlider("Thickness", function(x)
			getgenv().Aimbot.FOVSettings.Thickness = x
		end, { -- (options are optional)
			["min"] = 1, -- Default: 0
			["max"] = 5, -- Default: 100
			["readonly"] = false, -- Default: false
		...
		local Switch = Tab3:AddSwitch("Silent aim (may work on some games like arsenal)", function(bool)
			if bool then
		...
		local Switch = Tab3:AddSwitch("Spinbot", function(bool)
			if bool then
				spin()
			else
		...
		local Slider = Tab3:AddSlider("Spinbot speed", function(x)
			getgenv().spinSpeed = x
		end, { -- (options are optional)
			["min"] = 20, -- Default: 0
			["max"] = 500, -- Default: 100
		...
		local Switch = Tab3:AddSwitch("Third person (may work on some games)", function(bool)
			if bool then
				game:GetService"UserInputService".MouseBehavior = Enum.MouseBehavior.LockCenter
				game.Players.LocalPlayer.CameraMode = Enum.CameraMode.Classic game.Players.LocalPlayer.CameraMaxZoomDistance = 30 game.Players.LocalPlayer.CameraMinZoomDistance = 30
			else
		...
		local Switch = Tab3:AddSwitch("Lockmouse (for third person)", function(bool)
			if bool then
				game:GetService"UserInputService".MouseBehavior = Enum.MouseBehavior.LockCenter
			else
				game:GetService"UserInputService".MouseBehavior = Enum.MouseBehavior.Default
...
rconsoleprint('Kakah menu legit loaded')
rconsolename('Kakah menu')	